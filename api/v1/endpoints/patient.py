...
def create_patient(patient_data: PatientSchemaBase, db: Session = Depends(get_session)):
    return PatientService.create_patient(patient_data, db)

@router.get("/{patient_id}", response_model=PatientSchemaResponse)
def get_patient_by_id(patient_id: int, db: Session = Depends(get_session)): 
...
def update_patient(patient_id: int, patient_data: PatientSchemaBase, db: Session = Depends(get_session)):
